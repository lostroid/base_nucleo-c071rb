...
+ v_wait_penalty_count      : Penalty-based delay               "패널티에 따른 지연"
+ v_retry_max_count         : Retry count for response failure  "응답실패 재시도 횟수"
+ v_timeout_us_cfg          : Response timeout configuration    "응답 타임아웃 설정"
...
void f_Scheduler_Config(
    ...
    , tu32 v_wait_penalty_count             //+ Penalty-based delay               "패널티에 따른 지연"
    , tu32 v_retry_max_count                //+ Retry count for response failure  "응답실패 재시도 횟수"
    , tu32 v_timeout_us_cfg                 //+ Response timeout configuration    "응답 타임아웃 설정"
    , tu32 v_cycle_run_time_us              //+ Cycle time setting                "주기 타임 설정"
    , void *p_user_struct)                  //+ User structure registration       "사용자 구조체 등록"
{
    ps_sch_control->v_wait_penalty_count  = v_wait_penalty_count;   
    ps_sch_control->v_retry_max_count  = v_retry_max_count;
    ps_sch_control->v_timeout_us_cfg        = v_timeout_us_cfg;
    ps_sch_control->ps_user_struct          = p_user_struct;
    f_Base_Tick_Systick32_Stopwatch_Start(&ps_sch_control->s_Run_Time_Interval, v_cycle_run_time_us);
}
    ...
    , tu32 v_table_size)
{
    ...
    ps_sch_control->v_wait_down_count       = 0;
...
{
    // Check interval
    if(ps_sch_control-> v_wait_down_count != 0)
    {
        ps_sch_control->v_wait_down_count--;
        return;
    }
...
void f_Scheduler_Next(ts_Scheduler_Control *ps_sch_control)
{
    ...
    ps_sch_control->pf_func = ps_sch_control->ps_table_list[ps_sch_control->v_table_pos];
    ps_sch_control->v_case_step = 0;
    ps_sch_control->v_retry_down_count = ps_sch_control->v_retry_max_count;
}
...
void f_Scheduler_RunTime_Finish(ts_Scheduler_Control *ps_sch_control)
{
    ...
    {  
        ...
        ps_sch_control->v_time_run_avg100_us = v_time_us;  //+ Set initial value
        return;
    }
    ...
    //+ Calculate average
    if(ps_sch_control->v_time_run_avg100_us < v_time_us)
    { 
        v_time_us = ((v_time_us - ps_sch_control->v_time_run_avg100_us) / 100); 
        if(v_time_us == 0)
            { v_time_us++; }
        ps_sch_control->v_time_run_avg100_us += v_time_us; 
    }
    else
    {
        v_time_us = (ps_sch_control->v_time_run_avg100_us - v_time_us);
        if(v_time_us == 0)
            { v_time_us++; }
        if(ps_sch_control->v_time_run_avg100_us > v_time_us)
            { ps_sch_control->v_time_run_avg100_us -= v_time_us; }
        else
            { ps_sch_control->v_time_run_avg100_us = 0; }
    }
}
...
void f_Scheduler_LoopTime_Update(ts_Scheduler_Control *ps_sch_control)
{
    ...
    {  
        ...
        ps_sch_control->v_time_loop_avg100_us = v_time_us;  //+ Set initial value
        return;
    }
    ...
    //+ Calculate average
    if(ps_sch_control->v_time_loop_avg100_us < v_time_us)
    { 
        v_time_us = ((v_time_us - ps_sch_control->v_time_loop_avg100_us) / 100); 
        if(v_time_us == 0)
            { v_time_us++; }
        ps_sch_control->v_time_loop_avg100_us += v_time_us; 
    }
    else
    {
        v_time_us = (ps_sch_control->v_time_loop_avg100_us - v_time_us);
        if(v_time_us == 0)
            { v_time_us++; }
        if(ps_sch_control->v_time_loop_avg100_us > v_time_us)
            { ps_sch_control->v_time_loop_avg100_us -= v_time_us; }
        else
            { ps_sch_control->v_time_loop_avg100_us = 0; }
    }
}
...
void f_Scheduler_RunTime_Title_print(void)
{
    ...
    { 
        f_DBG_Print_String("\r\nSystem infomation \r\n[Schedlue name] [Run now/avg100/max(us)] [Loop now/avg100/max(us)]");
    }
}
...
void f_Scheduler_RunTime_Info_print(ts_Scheduler_Control *ps_sch_control)
{
    if(f_DBG_Get_Mode(m_DBG_MODE_SCHEDLUE) == m_YESNO_YES)
    {
        ...
        f_DBG_Print_String("/");
        f_DBG_Print_Dec32(ps_sch_control->v_time_run_avg100_us);
        f_DBG_Print_String("/");
        f_DBG_Print_Dec32(ps_sch_control->v_time_run_max_us);
        f_DBG_Print_String("] [");
        f_DBG_Print_Dec32(ps_sch_control->v_time_loop_now_us);
        f_DBG_Print_String("/");
        f_DBG_Print_Dec32(ps_sch_control->v_time_loop_avg100_us);
        f_DBG_Print_String("/");
