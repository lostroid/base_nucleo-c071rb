...
*
* Description:
* This is the LL(Low Level) RCC Control.
*
* Revision History:
*   2025-08-06  New.
********************************************************************/
...
#define d_ll_RCC_HSE_TIMEOUT   1000             //+ 1000 = 1ms
...
{
    f_LL_RCC_Systick_Enable();                  //+ Enable SysTick
    f_LL_Flash_Set_Latency01();                 //+ Modify flash latency
    f_LL_RCC_APB_Enable();                      //+ Enable APB
    f_LL_RCC_HSE_Enable();                      //+ Enable HSE
    if(f_LL_RCC_HSE_Ready() == 0)               //+ Check HSE RUN
        { f_LL_RCC_HSE_Switch_Select(); }
    else
        { f_LL_RCC_HSE_Disable(); }             //+ Fail
    f_LL_RCC_Clock_Selection();
    f_LL_RCC_Clock_Enable();                    //+ Periphearl clock enable
}
//===================================================================
/* LL RCC Module
-------------------------------------------------------------------*/
...
{
    tu32 v_time;
    f_LL_Tick_Systick32_Start(&v_time);
    while((RCC->CR & RCC_CR_HSERDY) == 0)           //+ HSE Ready
    {
        //+ Time Out 1ms */
        if(f_LL_Tick_Systick32_Finish(&v_time) > 1000)
            { return 1; }   //+ Failed
    }
    return 0;               //+ OK
}
