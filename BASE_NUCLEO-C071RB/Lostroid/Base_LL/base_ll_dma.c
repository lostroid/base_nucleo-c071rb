...
*
* Description:
* This is the LL(Low Level) dma Control.
*
* Revision History:
*   2025-08-06  New.
********************************************************************/
...
void f_LL_DMA_Init(void)
{
    ...
    f_LL_DMA_Setting(d_DMA1_UART1_TX, &USART1->TDR, v_ccr);

    v_ccr = DMA_CCR_MINC   /// Memory address count up              "메모리 주소 자동 증가"
          | DMA_CCR_CIRC   /// Circular mode                        "순환 구조 사용"
          | DMA_CCR_TEIE;  /// Transfer error interrupt enable      "송신 에러 인터럽트 활성"
    f_LL_DMA_Setting(d_DMA1_UART1_RX, &USART1->RDR, v_ccr);
    
    v_ccr = DMA_CCR_MINC   /// Memory address count up              "메모리 주소 자동 증가"
          | DMA_CCR_DIR    /// Memory -> Peripheral                 "메모리에서 주변장치로 전송"
          | DMA_CCR_TCIE   /// Transfer complete interrupt enable   "송신 완료 인터럽트 활성"
          | DMA_CCR_TEIE;  /// Transfer error interrupt enable      "송신 에러 인터럽트 활성"
    f_LL_DMA_Setting(d_DMA1_UART2_TX, &USART2->TDR, v_ccr);
    
    v_ccr = DMA_CCR_MINC   /// Memory address count up              "메모리 주소 자동 증가"
          | DMA_CCR_CIRC   /// Circular mode                        "순환 구조 사용"
          | DMA_CCR_TEIE;   /// Transfer error interrupt enable      "송신 완료 인터럽트 활성"
    f_LL_DMA_Setting(d_DMA1_UART2_RX, &USART2->RDR, v_ccr);

    f_LL_DMAMUX_Channel_ID_Set(d_DMAMUX1_UART1_TX, m_DMA_MUX_USART1_TX_DMA);
    f_LL_DMAMUX_Channel_ID_Set(d_DMAMUX1_UART1_RX, m_DMA_MUX_USART1_RX_DMA);
    f_LL_DMAMUX_Channel_ID_Set(d_DMAMUX1_UART2_TX, m_DMA_MUX_USART2_TX_DMA);
    f_LL_DMAMUX_Channel_ID_Set(d_DMAMUX1_UART2_RX, m_DMA_MUX_USART2_RX_DMA);
}
//===================================================================
/*### DMA Module
---------------------------------------------------------------------
